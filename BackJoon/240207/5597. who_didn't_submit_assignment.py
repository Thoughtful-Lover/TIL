...
submit = [0] * 28       # 제출자는 28명

for i in range(28):     # 28개의 숫자를 입력 받아 위에서 만든 리스트에 배정
    n = int(input())

    submit[i] = n

not_submit = []     # 제출하지 않은 출석번호를 저장할 빈 리스트 정의
for j in range(1, 31):
    if j not in submit:     # 제출하지 않은 출석번호를 리스트에 배정
        not_submit.append(j)

not_submit.sort()       # 정렬

print(not_submit[0])        # 둘 중 작은수
print(not_submit[1])        # 둘 중 큰수